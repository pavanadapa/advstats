...
class strat():
  ...
  def prop(N, P = None, data = None, x = None, y = None, aff = "Yes"):
    ...
    if data is not None and x is not None:
      return ((data[data[y] == aff].groupby(x).count()/data.groupby(x).count()).mul(N, axis = 0)).sum()/SumN
    else:
      return (data[data == aff].count()/data.count()*pd.Series(N)).sum()/SumN 
  ...
  def propvar(N, n = None, P = None, data = None, x = None, y = None, aff = "Yes"):
    ...
    if P is not None and N is not None:
      ...
      n = pd.Series(n)
      NSquared = pd.Series(NSquared)
      return (NSquared*(1-n/N)*(P*(1-P))/(n-1)).sum()/(SumNSquared)
    if data is not None and x is not None:
      n = data.groupby(x).count()
      P = (data[data[y] == aff].groupby(x).count()/n)
      return ((1-n.div(N, axis = 0))*(P*(1-P))/(n-1)).mul(NSquared, axis = 0).sum()/SumNSquared
    else:
      NSquared = pd.Series(NSquared)
      N = pd.Series(N)
      n = data.count()
